...
This project focuses on implementing object-relational mapping (ORM) in Python using SQLAlchemy. ORM is a technique that allows developers to interact with a relational database using an object-oriented paradigm. By mapping database tables to Python classes and their attributes, developers can manipulate database records using familiar Python syntax. SQLAlchemy is a popular ORM library in Python that provides tools for database manipulation, query building, and schema management. Additionally, this project leverages SQL for database querying and manipulation, and it employs object-oriented programming (OOP) principles to structure the Python codebase effectively.
...
**Ensure MySQL Server Version 8.0 Is Installed.**

### Installation Guide for MySQL 8.0 on Ubuntu 20.04 LTS

Before proceeding, ensure that your MySQL server is updated to version 8.0. To install MySQL 8.0 on Ubuntu 20.04 LTS, follow these steps:
...
To connect to your MySQL server, execute the following command:

```
$ sudo mysql
```

Upon successful execution, you will enter the MySQL monitor, confirming your connection to the server.
...
If you prefer working in a containerized environment, you can use **"container-on-demand"** to run MySQL. **In this container, the default credentials are** `root/root`**

- Request a container running Ubuntu 20.04.
- Connect either via SSH or through the Web terminal.
- Start MySQL within the container using the following command:
```
$ service mysql start                                                   
```
After starting MySQL, you can interact with it using SQL commands. For example:
...
It is essential to include comments in your SQL script file to provide context and explanation for the commands. Here's an example of how to add comments within an SQL file:
...
To create a Python Virtual Environment for installing specific dependencies for this Python project, follow these steps:
...
Before installing MySQLdb, ensure MySQL is installed. Refer to: **How install MySQL 8.0 in Ubuntu 20.04**
...
#### 4. Exiting the Python Interpreter

To exit from the Python interpreter after verifying the version information, you have a couple of options:

##### (a). Use the quit() function:

```
>>> quit()
```

##### (b). Press `Ctrl + D` (on Unix-like systems) or `Ctrl + Z` (on Windows) and then press **Enter.** This will also exit you from the Python interpreter.

Either method will successfully exit you from the Python interpreter.
...
'1.4.22'
```
#### 3. Exiting the Python Interpreter
...
You may encounter the following warning message:
```
